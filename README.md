...
[CI](https://github.com/ultralytics/sdk/actions/workflows/ci.yml) regenerates the Python SDK from the versioned contract with the `main` branch of [Ultralytics OpenAPI](https://github.com/ultralytics/openapi), then fails on contract mismatch or generated drift. Pushes to `main`, scheduled runs, and manual runs detect upstream contract changes without breaking unrelated pull requests. CI also formats and lints Python, compiles the package, builds its wheel, installs through the Git subdirectory boundary, and exercises representative sync and async requests against a mock transport. Package releases use exactly `MAJOR.MINOR.PATCH`. Generated drift that synchronization detects at an unchanged version increments the patch number automatically; an SDK-only change committed together with its regenerated output must set `python.version` in `openapi.config.json` to the next patch in the same pull request, or nothing publishes. Both publish [`ultralytics-platform`](https://pypi.org/project/ultralytics-platform/) to PyPI through trusted publishing.
