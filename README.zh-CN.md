...
[CI](https://github.com/ultralytics/sdk/actions/workflows/ci.yml) 使用版本化的契约和 [Ultralytics OpenAPI](https://github.com/ultralytics/openapi) 的 `main` 分支重新生成 Python SDK，并在契约不匹配或生成内容漂移时失败。计划任务和手动运行会检测上游契约变更，而不会中断无关的拉取请求。CI 还会检查 Python 格式和 lint、编译软件包、构建 wheel、通过 Git 子目录边界进行安装，并使用模拟传输测试具有代表性的同步和异步请求。软件包版本使用 `openapi.config.json` 中的 `python.version`（若已设置），否则使用契约的 `info.version`，格式均为 `MAJOR.MINOR.PATCH`。契约版本更新或同步检测到的生成内容漂移到达 `main` 后，会通过可信发布将 [`ultralytics-platform`](https://pypi.org/project/ultralytics-platform/) 发布到 PyPI；同步检测到漂移且生成版本未变化时，会自动递增补丁版本。仅 SDK 的改动若与重新生成的文件一起提交，必须在同一个拉取请求中将 `python.version` 设为下一个补丁版本，否则不会发布。
