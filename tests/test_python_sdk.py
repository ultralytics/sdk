...
def test_priority_resource_requests() -> None:
    ...
    with Platform(api_key="ul_test", http_client=http_client) as client:
        datasets = client.datasets.list("user", limit=2)
        client.models.retrieve("user", "project", "model")
    ...
    assert [(request.method, request.url.raw_path) for request in requests] == [
        ...
        ("GET", b"/api/models/user/project/model"),
        ("POST", b"/api/deployments/user"),
        ("POST", b"/api/training/start"),
        ("POST", b"/api/models/user/project/model/exports"),
    ]
    ...
    assert (
        requests[2].content
        == b'{"project":"project","model":"model","deployment":"production","name":"Production","region":"us-central1"}'
    )
...
def test_async_resource_tree() -> None:
    async def run() -> None:
        ...
        async with AsyncPlatform(api_key="ul_test", http_client=http_client) as client:
            datasets = await client.datasets.list("user", limit=1)

        assert requests[0].url == "https://platform.ultralytics.com/api/datasets/user?limit=1"
        assert datasets == {"ok": True}
